...
namespace StockRestAPI.Controllers
{
    ...
    public class DailyBarsController : Controller
    {
        ...
        public async Task<IActionResult> GetDailyBars(string ticker, DateTime startTime, DateTime endTime)
        {
            var dailyBars = await _context.DailyBars
                .Where(db => db.Symbol.ToUpper() == ticker.Trim().ToUpper() && db.Timestamp >= startTime.ToUniversalTime()) // Optional startTime
